...
public class DatabaseLockRegistry {
     ...
     */
    public int acquireLock(int databaseId, @NonNull SeekdbCompatDatabase database) throws Exception {
        synchronized (mLock) {
     ...
     */
    private void lockDatabase(final SeekdbCompatDatabase database) throws Exception {
        ...
        try {
            ...
            future = SqliteInspectionExecutors.submit(mExecutor, new Runnable() {
                @Override
                public void run() {
                    // starts a transaction (SeekDB may map/deny; see engine docs)
                    Cursor c = database.query(
                            new SimpleSQLiteQuery("BEGIN IMMEDIATE;"), cancellationSignal);
                    try {
                        c.getCount();
                    } finally {
                        c.close();
     ...
     */
    private void unlockDatabase(final SeekdbCompatDatabase database) throws Exception {
        final CancellationSignal cancellationSignal = new CancellationSignal();
        Future<?> future = null;
        try {
            // Submitting a Runnable, so we can set a timeout.
            future = SqliteInspectionExecutors.submit(mExecutor, new Runnable() {
                ...
                public void run() {
                    ...
                    Cursor c = database.query(
                            new SimpleSQLiteQuery("ROLLBACK;"), cancellationSignal);
                    try {
                        c.getCount();
                    } finally {
                        c.close();
    ...
    private static final class Lock {
        ...
        final SeekdbCompatDatabase mDatabase;
        int mCount = 0; // number of simultaneous locks secured on the database

        Lock(int lockId, int databaseId, SeekdbCompatDatabase database) {
            this.mLockId = lockId;
