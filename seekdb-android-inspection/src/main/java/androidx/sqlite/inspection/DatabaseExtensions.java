...
final class DatabaseExtensions {
    ...
    /** Thread-safe as {@link SeekdbCompatDatabase#getPath} and {@link Object#hashCode) are thread-safe.*/
    static String pathForDatabase(@NonNull SeekdbCompatDatabase database) {
    ...
    /** Thread-safe as {@link SeekdbCompatDatabase#getPath} is thread-safe. */
    static boolean isInMemoryDatabase(@NonNull SeekdbCompatDatabase database) {
        return Objects.equals(sInMemoryDatabasePath, database.getPath());
    }

    /**
     * Attempts to call {@link SeekdbCompatDatabase#acquireReference} on the provided object.
     *
     ...
     * {@link SeekdbCompatDatabase#acquireReference}.
     */
    static boolean tryAcquireReference(@NonNull SeekdbCompatDatabase database) {
        if (!database.isOpen()) {
            return false;
        }
