...
import com.oceanbase.seekdb.android.compat.SeekdbCompatDatabase;
import com.oceanbase.seekdb.android.compat.SeekdbCompatStatement;
import com.oceanbase.seekdb.android.compat.SeekdbInspectionBridge;
import com.oceanbase.seekdb.android.compat.SeekdbInspectorSchemaQuery;
...
final class SqliteInspector extends Inspector {
    ...
    /**
     * SeekDB uses MySQL/OceanBase catalogs; the stock SQLite query (sqlite_master / pragma_*) does
     * not run. Use {@link SeekdbInspectorSchemaQuery#FOR_APP_INSPECTION_GET_SCHEMA} so App
     * Inspection can list tables/columns.
     */
    private static final String sQueryTableInfo = SeekdbInspectorSchemaQuery.FOR_APP_INSPECTION_GET_SCHEMA;

    private static final Set<String> sHiddenTables = new HashSet<>(Arrays.asList(
            "android_metadata",
            "sqlite_sequence",
            // Room invalidation: SQLite often uses a TEMP table (not listed like base tables);
            // SeekDB compat rewrites it to a persistent table, so hide it for parity with SQLite UI.
    ...
    private void handleTrackDatabases(CommandCallback callback) {
        ...
        // Prefer explicit registration: findInstances() uses VMDebug heap walks that can return empty
        // on API 35+ when hidden-API linking denies VMDebug (see logcat "linking: denied").
        for (SeekdbCompatDatabase instance : SeekdbInspectionBridge.snapshotOpenDatabases()) {
            if (instance.isOpen()) {
        ...
        // Check for database instances in memory (best-effort when JVMTI allows)
        for (SeekdbCompatDatabase instance :
                mEnvironment.artTooling().findInstances(SeekdbCompatDatabase.class)) {
        ...
        // Check for database instances on disk (names under Context#getDatabasePath). SeekDB embed may
        // create non-app sibling files (etc/log/run/store) — do not register those as databases.
        // findInstances(Application) also depends on VMDebug; fall back to ActivityThread when empty.
        ArrayList<Application> applications = new ArrayList<>();
        for (Application instance : mEnvironment.artTooling().findInstances(Application.class)) {
            applications.add(instance);
        }
        if (applications.isEmpty()) {
            Application fallback = currentApplicationViaActivityThread();
            if (fallback != null) {
                applications.add(fallback);
            }
        }
        for (Application instance : applications) {
            for (String name : instance.databaseList()) {
                if (shouldSkipSeekDbStoreArtifactName(name)) {
                    continue;
                }
    ...
    /**
     * SeekDB storage layout can drop helper files next to the Room DB under databases/; they show up
     * in {@link Application#databaseList()} but are not openable SQL databases for inspection.
     */
    private static boolean shouldSkipSeekDbStoreArtifactName(@Nullable String name) {
        if (name == null) {
            return true;
        }
        switch (name) {
    ...
    @Nullable
    private static Application currentApplicationViaActivityThread() {
        try {
    ...
    private void registerDatabaseClosedHooks(EntryExitMatchingHookRegistry hookRegistry) {
        hookRegistry.registerHook(SeekdbCompatDatabase.class, ALL_REFERENCES_RELEASE_COMMAND_SIGNATURE,
                new EntryExitMatchingHookRegistry.OnExitCallback() {
                    ...
                    public void onExit(EntryExitMatchingHookRegistry.Frame exitFrame) {
                        ...
                        if (thisObject instanceof SeekdbCompatDatabase) {
                            onDatabaseClosed((SeekdbCompatDatabase) thisObject);
                        }
                    }
                });
    }
    ...
    private void registerDatabaseOpenedHooks() {
        // SeekDB uses SeekdbOpenHelper — there is no static SQLiteDatabase.openDatabase. Relying only
        // on ArtTooling.findInstances(SeekdbCompatDatabase) often yields an empty list on device;
        // hook the SupportSQLiteOpenHelper methods Room actually calls.
        Class<?> helperClass;
        try {
            helperClass = Class.forName("com.oceanbase.seekdb.android.compat.SeekdbOpenHelper");
        } catch (ClassNotFoundException e) {
            return;
        }
        final String writableSig = "getWritableDatabase()Landroidx/sqlite/db/SupportSQLiteDatabase;";
        final String readableSig = "getReadableDatabase()Landroidx/sqlite/db/SupportSQLiteDatabase;";
        ExitHook<Object> hook =
                new ExitHook<Object>() {
                    @Override
                    public Object onExit(Object result) {
                        try {
                            if (result instanceof SeekdbCompatDatabase) {
                                onDatabaseOpened((SeekdbCompatDatabase) result);
                            }
                        } catch (Exception exception) {
                            getConnection()
                                    .sendEvent(
                                            createErrorOccurredEvent(
                                                    "Unhandled Exception while processing an "
                                                            + "onDatabaseAdded event: "
                                                            + exception.getMessage(),
                                                    stackTraceFromException(exception),
                                                    null,
        ...
        mEnvironment.artTooling().registerExitHook(helperClass, writableSig, hook);
        mEnvironment.artTooling().registerExitHook(helperClass, readableSig, hook);
    }
        ...
        mEnvironment.artTooling().registerEntryHook(
                ...
                new EntryHook() {
                    ...
                    public void onEntry(@Nullable Object thisObject,
                        ...
                        if (thisObject instanceof SeekdbCompatDatabase) {
                            mDatabaseRegistry.notifyReleaseReference((SeekdbCompatDatabase) thisObject);
                        }
                    }
                });
    }
    ...
    /**
     * Triggering invalidation on {@link SeekdbCompatDatabase#endTransaction} allows us to avoid
     ...
     * {@link SeekdbCompatDatabase#setTransactionSuccessful} was called
     */
    private void registerInvalidationHooksTransaction(final RequestCollapsingThrottler throttler) {
        mEnvironment.artTooling().registerExitHook(SeekdbCompatDatabase.class, "endTransaction()V",
                new ExitHook<Object>() {
                    @Override
                    public Object onExit(Object result) {
     ...
     * <ul>
     *     <li>{@link SeekdbCompatStatement#execute}</li>
     *     <li>{@link SeekdbCompatStatement#executeInsert}</li>
     *     <li>{@link SeekdbCompatStatement#executeUpdateDelete}</li>
     * </ul>
     */
    private void registerInvalidationHooksSqliteStatement(
        ...
        for (String method : SQLITE_STATEMENT_EXECUTE_METHODS_SIGNATURES) {
            mEnvironment.artTooling().registerExitHook(SeekdbCompatStatement.class, method,
                    new ExitHook<Object>() {
                        @Override
                        public Object onExit(Object result) {
                            throttler.submitRequest();
    ...
    /**
     * sqlite-android hooked {@code rawQueryWithFactory}; SeekDB uses SupportSQLite query paths and
     * non-requery {@link Cursor} implementations. Rely on statement/transaction invalidation hooks.
     */
    ...
    private static Cursor rawQuery(@NonNull SeekdbCompatDatabase database, @NonNull String queryText,
            final String @NonNull [] params, @Nullable CancellationSignal cancellationSignal) {
        return database.query(new SimpleSQLiteQuery(queryText, params), cancellationSignal);
    }
    ...
    private @Nullable DatabaseConnection acquireConnection(int databaseId,
        ...
        SeekdbCompatDatabase database = mDatabaseRegistry.getConnection(databaseId);
        if (database == null) {
    ...
    private @NonNull Response querySchema(SeekdbCompatDatabase database) {
        Cursor cursor = null;
        try {
    ...
    void onDatabaseOpened(SeekdbCompatDatabase database) {
        mRoomInvalidationRegistry.invalidateCache();
        mDatabaseRegistry.notifyDatabaseOpened(database);
    }

    @SuppressWarnings("WeakerAccess") // avoiding a synthetic accessor
    void onDatabaseClosed(SeekdbCompatDatabase database) {
    ...
    static final class DatabaseConnection {
        final @NonNull SeekdbCompatDatabase mDatabase;
        final @NonNull Executor mExecutor;

        DatabaseConnection(@NonNull SeekdbCompatDatabase database, @NonNull Executor executor) {
            mDatabase = database;
            mExecutor = executor;
        }
    }
